...
namespace freeglutcallback
{
	...
	mouse_moved(int x, int y)
	{
		...
		//
		{
			...
			if (has_pull_performed)
			{
				...
				{
					globalvars::displaced_vertex_positions = globalvars::modi
							.deform(pulled_vert, globalvars::displaced_vertex_positions.col(pulled_vert));
				}

				// update positions (only the viewer)
				globalvars::viewer.get_mesh_buffer()
						.set_vertex_positions(globalvars::displaced_vertex_positions.cast<float>());
	...
	visualize_pressed(int)
	{
		...
		{
			std::vector<int> vert_ind = globalvars::modi
					.get_top_k_errors_edge_vertices(globalvars::num_entities_to_simplify);
			mohe::Mesh_connectivity::Defragmentation_maps defrag;
			globalvars::mesh.compute_defragmention_maps(defrag);
			if (!vert_ind.empty())
			{
