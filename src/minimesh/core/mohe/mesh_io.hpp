...
namespace minimesh
{
	namespace mohe
	{
		...
		public:
			...
			Mesh_connectivity& grid()
			{
				return _m;
			}
			const Mesh_connectivity& grid() const
			{
				return _m;
			}
			...
			void
			write_vtk_data(FILE* fl, std::vector<double>& data, const std::string name, const bool is_vector = false);
