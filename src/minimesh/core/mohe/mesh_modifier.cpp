...
// borrowed from https://github.com/icemiliang/loop_subdivision
		...
		)
		{
			...
			{
				he_at_v1_v2.twin().prev().twin().face().data().half_edge = he_at_v1_v2.twin().prev().twin().prev()
						.index();
			}
		...
		Mesh_modifier::simplify(int k)
		{
			...
			if (k > 0)
			{ printf("No valid edge to simplify. Simplification process will stop."); }
		}
		...
		std::pair<std::vector<Mesh_connectivity::Vertex_iterator>, std::vector<int>>
		Mesh_modifier::find_boundary_verts()
		{
		...
		void Mesh_modifier::parametrize_LSCM()
		{
			...
			std::pair<int, int> fixed_verts_ind = std::make_pair(boundary_v[ind.first].index(),
					boundary_v[ind.second].index());
			int small_fix_ind =
					fixed_verts_ind.first < fixed_verts_ind.second ? fixed_verts_ind.first : fixed_verts_ind.second;
			int large_fix_ind =
					fixed_verts_ind.first > fixed_verts_ind.second ? fixed_verts_ind.first : fixed_verts_ind.second;
			...
			for (int i = 0; i < F; ++i)
			{
				...
				do
				{
					...
					{
						W_elem.emplace_back(i * 6 + 2 * corner_count, 2 * defrag_map[P1.index()],
								rot_M(0, 0) - 1); // u1 * R11-1
						W_elem.emplace_back(i * 6 + 2 * corner_count, 2 * defrag_map[P1.index()] + 1,
								rot_M(0, 1)); //v1 * R12
						W_elem.emplace_back(i * 6 + 2 * corner_count + 1, 2 * defrag_map[P1.index()],
								rot_M(1, 0)); // u1 * R21
						W_elem.emplace_back(i * 6 + 2 * corner_count + 1, 2 * defrag_map[P1.index()] + 1,
								rot_M(1, 1) - 1);//v1 * R22 -1
					}
					...
					{
						W_elem.emplace_back(i * 6 + 2 * corner_count, 2 * defrag_map[P2.index()],
								-rot_M(0, 0)); // -R11 * u2
						W_elem.emplace_back(i * 6 + 2 * corner_count, 2 * defrag_map[P2.index()] + 1,
								-rot_M(0, 1)); // -R12 * v2
						W_elem.emplace_back(i * 6 + 2 * corner_count + 1, 2 * defrag_map[P2.index()],
								-rot_M(1, 0));// -R21 * u2
						W_elem.emplace_back(i * 6 + 2 * corner_count + 1, 2 * defrag_map[P2.index()] + 1,
								-rot_M(1, 1));// -R22 * v2
					}
			...
			while (itr < num_iter)
			{
				...
				for (int i = 0; i < N; ++i)
				{
					...
					do
					{
						...
						sum += w_ij * 0.5
								* (m_rotation[ring.half_edge().dest().index()] +
										m_rotation[ring.half_edge().origin().index()])
...
//    }
		...
		void Mesh_modifier::update_rotations(std::vector<Eigen::Matrix3d>& m_rots, Eigen::Matrix3Xd& deformed_pos)
		{
			for (int i = 0; i < mesh().n_total_vertices(); ++i)
			{
				...
				Eigen::JacobiSVD<Eigen::MatrixXd> svd(P * D * PPrime.transpose(),
