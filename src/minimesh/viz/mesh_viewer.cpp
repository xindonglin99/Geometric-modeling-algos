...
namespace minimesh
{
	...
	void Mesh_viewer::draw()
	{
		...
		glGetDoublev(GL_MODELVIEW_MATRIX, _model_view_matrix.data());
		gl_check_error();
		glGetDoublev(GL_PROJECTION_MATRIX, _projection_matrix.data());
		gl_check_error();
		glGetIntegerv(GL_VIEWPORT, _view_port.data());
		gl_check_error();
		...
		// Draw the spheres
		{
			...
			for (int i = 0; i < (int)_mesh_buffer.spheres_vertex_indices.size(); ++i)
			{
				...
				gl_begin(GL_POINTS);
				gl_check_error();
				glColor4fv(_mesh_buffer.spheres_colors.col(i).data());
				gl_check_error();
				glVertex3fv(vv.data());
				gl_check_error();
				gl_end();
		...
		if (_is_pulling_in_progress)
		{
			...
			Eigen::Vector3f point3 = screen2world_generic(point0.x(), point0.y(), 0, eye, eye, _view_port)
					.cast<float>();
			Eigen::Vector3f point4 = screen2world_generic(point1.x(), point1.y(), 0, eye, eye, _view_port)
					.cast<float>();
	...
	void Mesh_viewer::get_and_clear_vertex_displacement(bool& was_displaced, Eigen::Vector3f& disp_amount,
			int& pulled_vertex)
	{
		if (_is_pulling_in_progress)
		{
			...
			float* depth)
	{
		...
		glReadPixels(gl_click.x(), gl_click.y(), 1, 1, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, &stencil_value);
		gl_check_error();

		float depth_value;
		glReadPixels(gl_click.x(), gl_click.y(), 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &depth_value);
		gl_check_error();
